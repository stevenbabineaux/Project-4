...
// check for alignment errors
    ...
    ASSERT(tlb == NULL || pageTable == NULL || HPTable == NULL);	
    ASSERT(tlb != NULL || pageTable != NULL || HPTable != NULL);	
...
// from the virtual address
    ...
    int division = 0;
    if(HPTable != NULL)
    	division = currentThread->space->outerDivide;
	...
	if (vpn >= pageTableSize) {
	    ...
	    return AddressErrorException;
	} else if (!twoLevel && !pageTable[vpn].valid) {
	    DEBUG('a', "virtual page # %d too large for page table size %d!\n", 
			virtAddr, pageTableSize);
	    return PageFaultException;
	}
	 else if(twoLevel && !HPTable[vpn / division][vpn % division].valid){
	 	
		return PageFaultException;
	 }
	 
	if(twoLevel){
		entry = &HPTable[vpn / division][vpn % division];
	}
	else
		entry = &pageTable[vpn];
    } else {
